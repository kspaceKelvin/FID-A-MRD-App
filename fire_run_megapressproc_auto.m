...
% Updated by Jason Rock, UofT 2022 
% Updated by Kelvin Chow, Siemens Healthineers 2022
%
% USAGE:
% [diffSpec,sumSpec,subSpec1,subSpec2,outw]=fire_run_megapressproc_auto(raw,raww,coilcombos,avgAlignDomain,alignSS);
% 
% DESCRIPTION:
...
% 
% INPUTS:
% raw                = struct of io_loadspec_fire data
% raww               = struct of io_loadspec_fire water suppressed data
...
function [diffSpec,sumSpec,subSpec1,subSpec2,outw]=fire_run_megapressproc_auto(raw,raww,coilcombos,avgAlignDomain,alignSS)
...
%Jason Rock Edit 
% check for the water peak data and that the measurement ID matches
% suppressed data
% if(isfile(fullfile(tmpDir, 'out_w_scanner.mat')) && strcmp(load(fullfile(tmpDir, 'out_w_scanner.mat')).measurementID, out_scanner.measurementID)) 
%     water = true;
%     raww = load(fullfile(tmpDir, 'out_w_scanner.mat'));
%     disp('water unsuppressed data found from same measurement set as suppressed water scan');
% elseif(isfile(fullfile(tmpDir, 'out_w_scanner.mat')) && ~strcmp(load(fullfile(tmpDir, 'out_w_scanner.mat')).measurementID, out_scanner.measurementID))
%     disp('Water unsuppressed data found but it is not from same measurement set as water suppressed data');
%     disp('ommiting water unsuppressed data analysis');
%     water = false; 
% else
%     disp('cannot find water unsuppressed data');
%     disp('ommiting water unsuppressed data analysis');
%     water = false;
% end
...
if ~isempty(raww)
    ...
    disp('Starting without water unsuppressed data analysis');
    water = false;
end
...
if driftCorr=='n' || driftCorr=='N'
    ...
    while sat=='n' || sat=='N'
        ...
        set(h,'PaperUnits','centimeters');
        set(h,'PaperPosition',[0 0 20 15]);
        %saveas(h,fullfile('alignAvgs_prePostFig'),'jpg');
        % exportgraphics(h,'alignAverages.png','Resolution',1000);
        close(h);
...
set(h,'PaperUnits','centimeters');
set(h,'PaperPosition',[0 0 20 10]);
%saveas(h,fullfile('finalSpecFig'),'jpg');
% exportgraphics(h,'finalSpecFigHighRes.png','Resolution',1500);
