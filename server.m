...
%                     obj.tcpHandle = tcpserver('0.0.0.0',          obj.port, ...
%                                               'Timeout',          3000);  %#ok<TNMLP>  Consider moving toolbox function TCPIP out of the loop for better performance
        ...
        function handle(obj)
            try
                ...
                elseif strcmpi(config, "mapvbvd")
                    ...
                    recon = fire_mapVBVD;
                elseif strcmpi(config, "fida_megapress")
                    obj.log.info("Starting FID-A MEGA-PRESS processing based on config")
                    recon = fire_mapVBVD_fida;
